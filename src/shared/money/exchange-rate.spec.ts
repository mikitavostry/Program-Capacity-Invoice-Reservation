...
import { ExchangeRate, InvalidExchangeRateError, RateNotApplicableError } from './exchange-rate.js';
...
describe('ExchangeRate', () => {
  ...
  describe('rounding', () => {
    ...
    it('rounds to nearest when asked for a settlement figure instead', () => {
      const rate = ExchangeRate.of(EUR, USD, '1.095', AS_OF);

      expect(rate.convert(Money.fromDecimal('0.01', EUR), 'HALF_UP').toDecimalString()).toBe(
        '0.01',
      );
    });
    ...
    it('applies the same split into a currency with no minor unit', () => {
      ...
      expect(rate.convert(amount, 'HALF_UP').toDecimalString()).toBe('1502');
    });
    ...
    it('leaves an exact conversion alone under either policy', () => {
      ...
      expect(rate.convert(amount, 'HALF_UP').toDecimalString()).toBe('2.00');
    });
  });
