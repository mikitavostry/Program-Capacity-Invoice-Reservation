...
import { FixedClock, RecordingEventBus } from '../../../../test/support/in-memory-capacity.js';
...
import { OpenProgramHandler } from './open-program/open-program.handler.js';
...
describe('capacity handlers against Postgres', () => {
  ...
  function wire() {
    ...
    return {
      open: new OpenProgramHandler(transactions, readModel, clock, bus),
      reserve: new ReserveCapacityHandler(transactions, readModel, rates, clock, bus),
      repay: new RecordRepaymentHandler(transactions, clock, bus),
  ...
  beforeEach(async () => {
    ...
    handlers = wire();
    await handlers.open.execute(new OpenProgramCommand(PROGRAM, usd('1000.00')));
  });
  ...
  it('recognises a program opened twice through the database’s unique key', async () => {
    const repeat = await handlers.open.execute(new OpenProgramCommand(PROGRAM, usd('1000.00')));
    expect(repeat.created).toBe(false);

    await expect(
      handlers.open.execute(new OpenProgramCommand(PROGRAM, usd('5.00'))),
    ).rejects.toThrow(ProgramAlreadyExistsError);
  });
