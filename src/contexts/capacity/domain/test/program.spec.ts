...
import { Program, type ProgramSnapshot, type ProgramStatus } from '../program.js';
...
describe('Program', () => {
  ...
  describe('opening from a treasury message', () => {
    const state = () => ({
      creditLimit: usd('2500.00'),
      sequence: 7,
      at: OPENED_AT,
    });

    it('starts from the limit and sequence the message carries', () => {
      const program = Program.openFromTreasury(PROGRAM_ID, state());

      expect(program.creditLimit.equals(usd('2500.00'))).toBe(true);
      expect(program.reservedAmount.isZero).toBe(true);
      expect(program.treasurySequence).toBe(7);
      expect(program.pullDomainEvents()).toEqual([expect.any(ProgramOpened)]);
    });

    it('treats only a newer message as an update afterwards', () => {
      const program = Program.openFromTreasury(PROGRAM_ID, state());

      expect(() => program.applyTreasuryState({ ...state(), sequence: 7 })).toThrow(
        StaleTreasuryUpdateError,
      );
      expect(() => program.applyTreasuryState({ ...state(), sequence: 8 })).not.toThrow();
    });

    it('starts active unless treasury says otherwise', () => {
      expect(Program.openFromTreasury(PROGRAM_ID, state()).status).toBe('ACTIVE');
      expect(Program.openFromTreasury(PROGRAM_ID, { ...state(), status: 'SUSPENDED' }).status).toBe(
        'SUSPENDED',
      );
    });
  });
  ...
  describe('treasury updates', () => {
    ...
    const applyTreasury = (program: Program, creditLimit: Money, sequence: number): void =>
    ...
    const setStatus = (program: Program, status: ProgramStatus | null, sequence: number): void =>
      program.applyTreasuryState({
        creditLimit: null,
        sequence,
        status,
    ...
    describe('status', () => {
      it('suspends a program, which then refuses new reservations', () => {
        const program = openProgram('1000.00');

        setStatus(program, 'SUSPENDED', 1);
        const [event] = program.pullDomainEvents() as ProgramStatusChanged[];

        expect(program.status).toBe('SUSPENDED');
        expect(event).toBeInstanceOf(ProgramStatusChanged);
        expect(event).toMatchObject({ previousStatus: 'ACTIVE', status: 'SUSPENDED' });
        expect(event.treasurySequence).toBe(1);
        expect(() => reserve(program, usd('10.00'))).toThrow(ProgramNotActiveError);
      });

      it('reactivates a suspended program', () => {
        const program = openProgram('1000.00');
        setStatus(program, 'SUSPENDED', 1);

        setStatus(program, 'ACTIVE', 2);

        expect(program.status).toBe('ACTIVE');
        expect(() => reserve(program, usd('10.00'))).not.toThrow();
      });

      it('leaves the limit alone when the message carries only a status', () => {
        const program = openProgram('1000.00');

        setStatus(program, 'SUSPENDED', 1);

        expect(program.creditLimit.equals(usd('1000.00'))).toBe(true);
        expect(program.treasurySequence).toBe(1);
      });

      it('records nothing when the status is unchanged', () => {
        const program = openProgram('1000.00');

        setStatus(program, 'ACTIVE', 1);

        expect(program.domainEvents).toHaveLength(0);
      });

      it('leaves the status alone when the message does not state one', () => {
        const program = openProgram('1000.00');
        setStatus(program, 'SUSPENDED', 1);

        setStatus(program, null, 2);

        expect(program.status).toBe('SUSPENDED');
      });

      it('ignores a status from a message older than the state already applied', () => {
        const program = openProgram('1000.00');
        setStatus(program, 'SUSPENDED', 5);

        expect(() => setStatus(program, 'ACTIVE', 4)).toThrow(StaleTreasuryUpdateError);
        expect(program.status).toBe('SUSPENDED');
      });
    });
    ...
    it('refuses a limit in another currency, or one that is not positive', () => {
      ...
      expect(() => applyTreasury(program, eur('2000.00'), 7)).toThrow(
        TreasuryCurrencyMismatchError,
      );
