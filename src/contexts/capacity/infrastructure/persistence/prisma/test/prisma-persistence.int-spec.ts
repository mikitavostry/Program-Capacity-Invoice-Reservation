...
describe('Prisma persistence', () => {
  ...
  describe('database constraints, as a last line of defence', () => {
    ...
    it('allows reserved to exceed the limit, which is how a treasury limit cut lands', async () => {
      ...
      // them, and the program simply takes no new reservations (docs/architecture.md §2.5).
      await expect(
    ...
    it('refuses to change the currency of a program that holds reservations', async () => {
      const id = await openProgram();
      await reserve(id, usd('100.00'));

      // Cascading would relabel the held dollars as euros without converting them.
      await expect(
        prisma.$executeRaw`UPDATE programs SET currency = 'EUR' WHERE id = ${id.value}`,
      ).rejects.toThrow(/_currency_fkey/);
      await expect(
        prisma.reservation.findFirstOrThrow({ where: { programId: id.value } }),
      ).resolves.toMatchObject({ reservedCurrency: 'USD' });
    });
